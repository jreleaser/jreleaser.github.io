...
== Maven Multiple Subprojects

In a maven multiple subproject setting, you can control the projects that are going to be deployed by setting the `maven.deploy.skip` property in the root and subproject pom's.
...
You also need to add `--non-recursive` to the `jreleaser:deploy` goal so that the staged artifacts are processed and uploaded only once.

[source]
----
$ mvn jreleaser:deploy --non-recursive
----
