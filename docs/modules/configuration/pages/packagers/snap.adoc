...
NOTE: Only artifacts with `.tbz2`, `.tgz`, `.txz`, `.tar.gz`, `.tar.xz`, `tar.bz2`, `.tar`, or `.jar` extension may be packaged with Snapcraft.
...
If there is more than on matching artifact in a given distribution you may add a `skipSnap` extra property to the target
