...
NOTE: Only artifacts with `.tbz2`, `.tgz`, `.txz`, `.tar.gz`, `.tar.xz`, `tar.bz2`, or `.tar` extension may be packaged with Spec.
...
If there is more than on matching artifact in a given distribution you may add a `skipSpec` extra property to the target
