...
JReleaser calculates the value of the containing app directory when packaging a xref:ROOT:distributions/native-package.adoc[]
...
If there is more than one matching artifact in a given distribution you may add a `skipMacports` extra property to the target
