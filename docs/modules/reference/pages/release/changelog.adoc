...
Scoped commits may be categorized as well by defining an extra property named `categorizeScopes` with
a `true` value.
...
NOTE: The `conventional-commits` preset must be configured.
