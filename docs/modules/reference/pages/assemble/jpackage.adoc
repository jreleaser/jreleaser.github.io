...
:java-assembler: true
:platform:       true
:assembler_sys_key: jpackge
:assembler_env_key: JPACKAGE
...
include::partial$assemble/yaml/common-head.adoc[]
      ...
      windows:
        ...
        # icon:dot-circle[] icon:file-alt[]
        helpUrl: pass:[https://acme.com/help]

        # URL of available application update information.
        # icon:dot-circle[] icon:file-alt[]
        updateUrl: pass:[https://acme.com/update]
...
include::partial$assemble/toml/common-head.adoc[]
  ...
  [assemble.{assembler_id}.app.applicationPackage]
    ...
    # can be used to describe the association.
    # icon:dot-circle[]
    fileAssociations: = ["path/to/file.properties"]
...
include::partial$assemble/maven/common-head.adoc[]
        ...
        <windows>
          ...
          <!--
            ...
            icon:dot-circle[]
          -->
          <shortcutPrompt>true</shortcutPrompt>

          <!--
            URL where user can obtain further information or technical support.
            icon:dot-circle[] icon:file-alt[]
          -->
          <helpUrl>pass:[https://acme.com/help]</helpUrl>

          <!--
            URL of available application update information.
            icon:dot-circle[] icon:file-alt[]
          -->
          <updateUrl>pass:[https://acme.com/update]</updateUrl>
...
you may have JReleaser assemble a runtime image via xref:reference:assemble/jlink.adoc[] and consume it with this
assembler. The following snippet shows how to configure a non-exported `jlink` assembly named `app-jlink` which is consumed
by a `jpackage` assembly named `app`. This in turns creates a xref:concepts:distributions/native-package.adoc[] distribution
named `app`.
