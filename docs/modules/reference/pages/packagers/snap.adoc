...
include::partial$packagers/yaml/package-name.adoc[]
    ...
    # Configures build/run platforms
    # More details can be found at link:https://ubuntu.com/docs/snapcraft/9/reference/platforms/[]
    # Default is empty
    # icon:dot-circle[]
    platforms:
      amd64:
...
include::partial$packagers/toml/package-name.adoc[]
  ...
  # Default is empty
  # icon:dot-circle[]
  pass:[[[packagers.snap.platforms.amd64]]]
    # icon:exclamation-triangle[]
    buildOn = ["amd64"]
    # icon:dot-circle[]
    buildFor = ["amd64"]
